...
model_1 = DCN_Inception().to(device)   
...
model_1.load_state_dict(torch.load(pretrained_model_DCN_Inception, map_location='cpu'))
...
# def test( model1, model2, device, epsilon ):
    ...
    for index_input in range(start_idx, end_idx): # idx of source images to be attacked in the dataset
        ...
        if(image_space_attack==True):
            ...
            # loss = wei_KL * criterion_KL(output, target)
        if(Used_attack_method_idx == 12 or Used_attack_method_idx == 13): # extended C&W attack
            criterion_percp = nn.MSELoss() # L2 loss is mean square error loss
            # loss = max(wei_KL * criterion_KL(output, target), 0) + B_CONST * criterion_percp(original_source_image, )
            # output = torch.tanh(output) * 0.5 + 0.5 #XXXXXXXXXXXXXXXXXXX
            # target = torch.tanh(target) * 0.5 + 0.5 #XXXXXXXXXXXXXXXXXXx
            loss = max(wei_KL * criterion_KL(output, target), 0) # in the first step, we do not use perceptual loss, We use it in following iteration steps. Because there is no adv. exp in the 1st iteration
