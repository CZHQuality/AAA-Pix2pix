...
# method_to_import = 'attack_method_' + str(Used_attack_method_idx)

# from Attack_methods_library import saliency_attack_1_start, saliency_attack_1 as saliency_attack_start, saliency_attack
# from Attack_methods_library import attack_method_4 as attack_used # select attack method from library
# from Attack_methods_library import method_to_import as attack_used # select attack method from library
...
model_1.load_state_dict(torch.load(pretrained_model_GazeGAN_2, map_location='cpu'))
...
# def test( model1, model2, device, epsilon ):
    ...
    for index_input in range(start_idx, end_idx): # idx of source images to be attacked in the dataset
        ...
        data, target, target_NSS, data_target = data.to(device), target.to(device), target_NSS.to(device), data_target.to(device)
        ...
        if(feature_space_attack==True):
            ...
            # loss = wei_Repre * criterion_KL(d6, d6_target) + wei_L1 * criterion_L1(output, target) + wei_CC * criterion_CC(output, target) + wei_KL * criterion_KL(output, target) + wei_NSS * criterion_NSS(output, target_NSS) 
        ...
        if((Used_attack_method_idx == 12 or Used_attack_method_idx == 13) and feature_space_attack==True): # extended C&W attack
            criterion_percp = nn.MSELoss() # L2 loss is mean square error loss
            # loss = max(wei_KL * criterion_KL(output, target), 0) + B_CONST * criterion_percp(original_source_image, )
            loss = max(wei_KL * criterion_KL(d6, d6_target), 0) # in the first step, we do not use perceptual loss, We use it in following iteration steps. Because there is no adv. exp in the 1st iteration
        ...
        for step in range(1, iter_num): # we set the iteration steps as 50
            ...
            data_step, target, target_NSS, data_target, d6_target = data_step.to(device), target.to(device), target_NSS.to(device), data_target.to(device), d6_target.to(device)
            ...
            # loss_step = wei_L1 * criterion_L1(output_step, target) + wei_CC * criterion_CC(output_step, target) + wei_KL * criterion_KL(output_step, target) + wei_NSS * criterion_NSS(output_step, target_NSS) + wei_Repre * criterion_Repre(d6_step, d6_target, d5_step, d5_target)  
            if(feature_space_attack == True and Used_attack_method_idx != 10 and Used_attack_method_idx != 12):
                loss_step = wei_Repre * criterion_KL(d6_step, d6_target)  # data_step is the iteratively updated adv. example 
            if(image_space_attack == True and Used_attack_method_idx != 10 and Used_attack_method_idx != 12):
                # loss_step = wei_L1 * criterion_L1(output_step, target) + wei_CC * criterion_CC(output_step, target) + wei_KL * criterion_KL(output_step, target) + wei_NSS * criterion_NSS(output_step, target_NSS) + wei_percep * criterion_L1(data_step, original_source_image)
                loss_step = wei_KL * criterion_KL(output_step, target)
            if(use_L1_perceptual == True and Used_attack_method_idx != 10 and Used_attack_method_idx != 12): 
            ...
            if(Used_attack_method_idx == 10 and image_space_attack == True): # Used_attack_method_idx == 10 is hot/cold attack, which uses different loss compared other attacks. Only supports image-space attack
                ...
                # loss_step = wei_KL * criterion_KL(output_step, target) + wei_percep * criterion_percp(data_step, original_source_image)
                # print("KL loss and perceptual loss are:", wei_KL*criterion_KL(output_step, target), wei_percep_SSIM * criterion_percp(data_step, original_source_image))
            ...
            if((Used_attack_method_idx == 12 or Used_attack_method_idx == 13) and feature_space_attack == True): # extended C&W attack
                criterion_percp = nn.MSELoss() # L2 loss is mean square error loss
                # wei_percep = B_CONST
                # original_source_image = torch.tanh(original_source_image) * 0.5 + 0.5 # for C&W attack, transfering into tanh space as an adaptive constraint
            ...
            # perturbed_data_step = saliency_attack(data_step, epsilon_step, data_step_grad)
            if(Used_attack_method_idx == 1 or Used_attack_method_idx == 7 or Used_attack_method_idx == 9 or Used_attack_method_idx == 13):
            ...
            elif(Used_attack_method_idx == 2 or Used_attack_method_idx == 8 or Used_attack_method_idx == 12):
                beta_2m = 0.9
                ...
                pass
            elif(Used_attack_method_idx == 14):
                beta_2m = 0.9
